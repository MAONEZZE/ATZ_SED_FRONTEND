...
describe("answerKeyForField", () => {
  it("mapeia campos fixos para as chaves do backend", () => {
    ...
    expect(answerKeyForField({ label: "Telefone", type: "phone" })).toBe(
      "telefone",
    );
  });
  ...
  it("usa o tipo quando a label difere", () => {
    ...
    expect(answerKeyForField({ label: "WhatsApp", type: "phone" })).toBe(
      "telefone",
    );
  });

  it("campos customizados usam a própria label", () => {
    expect(answerKeyForField({ label: "Endereço", type: "text" })).toBe(
      "Endereço",
    );
    expect(answerKeyForField({ label: "Instagram", type: "text" })).toBe(
      "Instagram",
    );
  });
});
